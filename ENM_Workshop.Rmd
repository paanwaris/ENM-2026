...
# 1) terra comes from CRAN, the official R package repository:
install.packages("terra")

# 2) nicheR and bean live on GitHub, so we install them with the 'remotes' helper:
install.packages("remotes")
remotes::install_github("EscobarLab/nicheR")   # Module A  (Day 1)
remotes::install_github("EscobarLab/bean")      # Module B  (Day 2)
```

::: {.warning}
**First-time install tips.** GitHub installs can ask to update other packages —
choosing "None" is usually fine for a workshop. On Windows you may be prompted
to install **Rtools**; on macOS you may need the **Xcode command-line tools**.
If an install fails, see [Troubleshooting](#troubleshooting).
:::
...
**`install_github()` fails.** Make sure `remotes` is installed
(`install.packages("remotes")`). On Windows install **Rtools**; on macOS install
the Xcode command-line tools with `xcode-select --install` in a terminal.
