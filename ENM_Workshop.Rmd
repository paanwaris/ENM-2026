...
```{r a-reshape}
# The safe covariance range for this niche's variable pair:
generalist$cov_limits

# A magnitude that is always valid is anything below sqrt(var1 * var2).
# We take half of that, which corresponds to a correlation of about 0.5:
vars     <- diag(generalist$cov_matrix)          # the two marginal variances
safe_cov <- 0.5 * sqrt(vars[[1]] * vars[[2]])

# Tilt the SAME niche two ways — without rebuilding it from ranges:
gen_pos <- update_ellipsoid_covariance(generalist, covariance =  safe_cov)  # uphill
gen_neg <- update_ellipsoid_covariance(generalist, covariance = -safe_cov)  # downhill
...
```{r a-reshape-plot, fig.width=7, fig.height=5.5}
# Compare the original (grey) with the two tilted versions
plot_ellipsoid(
  generalist,
  background = back_data[, c("bio_1", "bio_12")],
  col_ell = "#7f8c8d", col_bg = "grey88", lwd = 2.5, pch = 20, cex_bg = 0.35,
  xlab = "bio_1 (temperature)", ylab = "bio_12 (precipitation)",
  main = "Same niche, reshaped: zero (grey) / positive (blue) / negative (pink)"
)
add_ellipsoid(gen_pos, col_ell = "#5E78C4", lwd = 2.5)   # positive covariance
add_ellipsoid(gen_neg, col_ell = "#D9559E", lwd = 2.5)   # negative covariance
```

::: {.seealso}
**What you should see.** Three ovals sharing the same centre: the grey one
upright, the blue one tilted uphill, the pink one tilted downhill — exactly like
the schematic above. Because everything is recomputed internally, `gen_pos` and
`gen_neg` are ready to hand straight to `predict()`.
:::
...
## Turn a niche into virtual sightings — with & without bias {#a-sample}

To connect theory to real data, `nicheR` can *sample* virtual occurrence points
from a prediction with **`sample_data()`**. This is perfect for teaching and for
testing methods — and it is exactly how you can **create a biased dataset on
purpose** to hand to Module B's bias correction.
...
- **`bias`** — an optional **bias layer** (a raster of relative sampling effort).
  With no bias, points follow *true* suitability; with a bias layer, they are
  pulled toward the over-sampled/accessible places — just like real field data.

<p align="center"><img src="images/concepts/concept_sampling_bias_layer.png" width="94%"></p>

**First, draw an *unbiased* sample** — points that honestly track suitability:
...
**Now load the bias layer that ships with `nicheR`** and draw a *biased* sample.
(We look the file up by name so it keeps working regardless of the exact filename.)

```{r a-sample-load-bias}
# Find and open the example bias raster from nicheR's extdata folder:
bias_file  <- list.files(system.file("extdata", package = "nicheR"),
                         pattern = "bias", full.names = TRUE)[1]
bias_layer <- terra::rast(bias_file)

bias_layer          # a SpatRaster of relative sampling effort (higher = sampled more)
terra::plot(bias_layer, main = "nicheR example bias layer (sampling effort)")
```

::: {.warning}
**One name to confirm.** I inferred that `sample_data()` takes the bias raster
through an argument called **`bias`**. If your version uses a different name
(e.g. `bias_layer`), change that single word in the next chunk.
:::

```{r a-sample-biased}
# Same niche, but now sampling effort is skewed by the bias layer:
gen_occ_biased <- sample_data(
  n_occ            = 100,
  prediction       = as.data.frame(gen_pred, xy = TRUE),
  prediction_layer = "suitability_trunc",
  sampling         = "random",
  method           = "suitability",
  bias             = bias_layer,   # <- the bias layer does the skewing
  seed             = 1
)
head(gen_occ_biased)
```

**Compare the two samples on the map:**

```{r a-sample-compare, fig.width=7, fig.height=6}
terra::plot(gen_pred[["suitability_trunc"]],
            main = "Unbiased (blue) vs. biased (red) virtual sightings")
points(gen_occ$x,        gen_occ$y,        pch = 20, col = "#5E78C4")  # unbiased
points(gen_occ_biased$x, gen_occ_biased$y, pch = 20, col = "#c0392b")  # biased
```

::: {.seealso}
**What you should see.** The blue (unbiased) points spread across the suitable
core, while the red (biased) points bunch up wherever the bias layer is high —
the same clumping problem Module B exists to fix. You have just *manufactured*
biased data to practise on.
:::
...
| `update_ellipsoid_covariance()` | tilt / correlate an existing niche (rotation) without rebuilding |
