...
We pull each sample's climate values out of `ma_bios` with `terra::extract()` and
draw them over the niche ellipse, one panel per strategy:

```{r a-sample-espace, fig.width=9, fig.height=3.6}
# Look up bio_1 & bio_12 at each sampled location ([, -1] drops the ID column):
env_of <- function(occ) {
  terra::extract(ma_bios[[c("bio_1", "bio_12")]], occ[, c("x", "y")])[, -1]
}
...
::: {.seealso}
**What you should see.** In **E-space** (top row) the strategy is obvious: blue
`"centroid"` points cluster at the niche centre, gold `"edge"` points hug the
ellipse boundary, and red `"random"` points fill the whole niche. In **G-space**
(bottom row) the same points map back onto the landscape — clustered in the
suitable core, ringing its margin, or scattered across it. Three ways to turn a
niche back into occurrence data.
:::
...
The final question: **does removing the bias change the predicted map?** We fit a
niche to the **original (biased)** occurrences, fit another to the **thinned**
occurrences, project both across Thailand, and put the two suitability maps side
by side in G-space.

```{r b-compare, fig.width=9, fig.height=5}
library(nicheR)   # provides the predict() method for bean ellipsoids

# 1) Fit a niche to the ORIGINAL (biased, un-thinned) occurrences:
fit_biased <- fit_ellipsoid(
  data     = prepared,
  env_vars = c("bio_1", "bio_12"),
  method   = "covmat",
  level    = 0.95
)
# (`fit`, the THINNED niche, was already created in Step 4.)

# 2) Project BOTH niches onto Thailand (same scaling as Step 5):
env_s        <- scale(env[[c("bio_1", "bio_12")]])
suit_biased  <- predict(fit_biased, newdata = env_s, suitability_truncated = TRUE)
suit_thinned <- predict(fit,        newdata = env_s, suitability_truncated = TRUE)

# 3) Compare the two suitability maps side by side:
par(mfrow = c(1, 2))
terra::plot(suit_biased[["suitability_trunc"]],
